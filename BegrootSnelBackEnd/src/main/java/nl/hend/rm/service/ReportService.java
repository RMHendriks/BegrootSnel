...
public class ReportService {
    ...
    public TransactionPeriodReportDto getReportByMonthYear(
        int year,
        int month
    ) {
    ...
    public TransactionPeriodReportDto getReportByDates(
        LocalDate startDate,
        LocalDate endDate
    ) {
        ...
        for (Category category : categoryList) {
            List<TransactionSplitDto> transactionSplitDtoList =
                getTransactionSplitDtoByCategoryAndDate(
                    category,
                    startDate,
                    endDate
                );
            BigDecimal budgetedAmount = BigDecimal.valueOf(0); // TODO Implement the budgeting
            TransactionCategoryGroupDto dto = new TransactionCategoryGroupDto(
                category,
                transactionSplitDtoList,
                budgetedAmount
            );
        ...
        return new TransactionPeriodReportDto(
            startDate,
            endDate,
            transactionCategoryGroupDtoList
        );
    }

    private List<TransactionSplitDto> getTransactionSplitDtoByCategoryAndDate(
        Category category,
        LocalDate startDate,
        LocalDate endDate
    ) {
        List<TransactionSplit> dbQueryList =
            Transaction.findTransactionListByDateAndCategory(
                category,
                startDate,
                endDate
            );
    ...
    private TransactionSplitDto mapToTransactionSplitDto(TransactionSplit s) {
        ...
        List<TransactionSplit> othersSplits = t.splits
            .stream()
        ...
        return new TransactionSplitDto(
            ...
            t.account != null ? t.account.accountNumber : null, // accountNumber
